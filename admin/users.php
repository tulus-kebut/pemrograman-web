...
if (isset($_GET['toggle'])) {
    $uid = (int)$_GET['toggle'];
    if ($uid !== (int)$_SESSION['user_id']) $db->prepare('UPDATE users SET is_active = NOT is_active WHERE id = ?')->execute([$uid]);
...
if (isset($_GET['make_user'])) {
    ...
    if ($uid !== (int)$_SESSION['user_id']) $db->prepare("UPDATE users SET role='user' WHERE id=?")->execute([$uid]);
    header('Location: users.php'); exit;
}
if (isset($_GET['delete'])) {
    $uid = (int)$_GET['delete'];
    if ($uid !== (int)$_SESSION['user_id']) $db->prepare('DELETE FROM users WHERE id = ?')->execute([$uid]);
    header('Location: users.php'); exit;
}
...
$sql = "SELECT u.*,
        ...
        FROM users u";
$params = [];
if ($search) { $sql .= " WHERE u.username LIKE ? OR u.email LIKE ?"; $params = ["%$search%","%$search%"]; }
        ...
        <tr>
          ...
          <td>
            <div class="tbl-acts">
              ...
              <?php else: ?>
                <a href="users.php?make_admin=<?= $u['id'] ?>" title="Jadikan Admin" onclick="return confirm('Jadikan Admin?')"><i class="ti ti-arrow-up"></i></a>
              <?php endif; ?>
              <a href="users.php?toggle=<?= $u['id'] ?>" title="<?= $u['is_active']?'Nonaktifkan':'Aktifkan' ?>"><i class="ti ti-<?= $u['is_active']?'ban':'check' ?>"></i></a>
              <a href="users.php?delete=<?= $u['id'] ?>" class="del" title="Delete" onclick="return confirm('Hapus user ini permanen?')"><i class="ti ti-trash"></i></a>
            </div>
          </td>
        </tr>
      <?php endforeach; ?>
