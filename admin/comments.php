...
if (isset($_GET['delete_comment'])) {
    $db->prepare('DELETE FROM comments WHERE id = ?')->execute([(int)$_GET['delete_comment']]);
    header('Location: comments.php?tab=comments'); exit;
}

// ── Delete post ──
if (isset($_GET['delete_post'])) {
    $db->prepare('DELETE FROM posts WHERE id = ?')->execute([(int)$_GET['delete_post']]);
    header('Location: comments.php?tab=posts'); exit;
}

// ── Delete reply ──
if (isset($_GET['delete_reply'])) {
    $db->prepare('DELETE FROM post_replies WHERE id = ?')->execute([(int)$_GET['delete_reply']]);
    header('Location: comments.php?tab=posts'); exit;
}
...
<div class="admin-layout">
  ...
  <div class="admin-content">
    ...
    <table class="data-table">
      ...
      <tbody>
      <?php if (!$comments): ?>
        <tr><td colspan="6" style="text-align:center;color:var(--dark);padding:24px">Belum ada komentar.</td></tr>
      <?php else: foreach ($comments as $c): ?>
        <tr>
          ...
          <td>
            <div class="tbl-acts">
              <a href="comments.php?delete_comment=<?= $c['id'] ?>" class="del" title="Delete" onclick="return confirm('Hapus komentar ini?')"><i class="ti ti-trash"></i></a>
            </div>
          </td>
        </tr>
    ...
    <table class="data-table">
      ...
      <tbody>
      <?php if (!$posts): ?>
        <tr><td colspan="7" style="text-align:center;color:var(--dark);padding:24px">Belum ada post komunitas.</td></tr>
      <?php else: foreach ($posts as $p): ?>
        <tr>
          ...
          <td>
            <div class="tbl-acts">
              <a href="../community.php#post-<?= $p['id'] ?>" target="_blank" title="View"><i class="ti ti-eye"></i></a>
              <a href="comments.php?delete_post=<?= $p['id'] ?>" class="del" title="Delete" onclick="return confirm('Hapus post ini beserta semua balasannya?')"><i class="ti ti-trash"></i></a>
            </div>
          </td>
        </tr>
      <?php endforeach; endif; ?>
